...
class I3D_Classifier(nn.Module):
    def __init__(self, num_classes=101, dropout_drop_prob=0.5, spatial_squeeze=True, use_sim=False):
        super(I3D_Classifier, self).__init__()
    ...
    def forward(self, x):
        x = self.i3d(x)
        if self.use_sim:
            x_embed = F.adaptive_avg_pool3d(x, output_size=1)
            x_embed = x_embed.view(x_embed.size(0), -1)
            x_embed = self.embed(x_embed)
        x = self.pool(x)
        x = self.drop(x)
        x = self.classifier(x)
        ...
        if self.use_sim:
            return averaged_logits, x_embed
        else:
...
if __name__ == "__main__":
    ...
    inputs = torch.rand(4, 3, 64, 112, 112)
